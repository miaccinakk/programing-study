      ...
      advanced: [
        {
          ...
          body: "Интеграционные тесты проверяют связку модулей (например контроллер + сервис + реальная тестовая БД в Docker). e2e через Playwright гоняют реальный браузер по сценарию пользователя. В NestJS e2e делают через supertest и тестовый модуль приложения. Данные между тестами изолируют (транзакция с откатом или чистка БД).",
        },
        {
          type: "code",
          lang: "ts",
