...
// useMemo: мемоизация тя��ёлого вычисления
  ...
  return debounced
        ...
        },
        {
          ...
          items: [
            ...
            "useReducer для сложной логики состояния с ��есколькими действиями",
...
export async function GET(req: Request) {
  ...
  return Response.json(users)
        ...
        },
        {
          ...
          body: 'Server Actions — функции с директивой "use server", которые вызываются с клиента, но выполняются на сервере. Позволяют делать мутации (создать/обновить/удалить) прямо из формы без ручного написания API-роута, с прогрессивным улучшением и типобезопасн��стью.',
        },
        {
          type: "code",
          lang: "tsx",
...
await prisma.post.create({ data: { title: "Hi", authorId: 1 } })`,
      ...
      middle: [
        ...
        },
        {
          ...
          body: "Транзакция — группа операций по принципу «всё или ничего». ACID: Atomicity (атом��рность), Consistency (согласованность), Isolation (изоляция от других транзакций), Durability (сохранность после коммита). Если внутри что-то падает — происходит откат (ROLLBACK), и БД остаётся в согласованном состоянии.",
        },
        {
          type: "code",
          lang: "ts",
...
volumes:
  pgdata: {}`,
        ...
        },
        {
          ...
          body: "docker compose up -d поднимает весь стек в фоне, docker compose down останавливает и удаляет контейнеры (но volumes по умолчанию остаются). Со временем накапливается мусор — остановленные контейнеры, «висячие» (dangling) образы, неиспользуемые тома. Их чистят командами prune. Это частый вопрос на собеседовании: «диск кончился — что делать».",
        },
        {
          type: "table",
...
// 3) подмешиваем найденное в промпт и генерируем ответ`,
        },
        {
          ...
          items: [
            ...
            "Оце��ка качества (evals), защита от prompt injection, лимиты и стоимость",
          ],
        },
        {
          type: "callout",
